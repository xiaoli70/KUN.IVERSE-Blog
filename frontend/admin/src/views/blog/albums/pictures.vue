		...
		<el-card class="w100">
			<el-row class="mb20">
				<el-col :span="24">
					...
					> -->
					<UploadImg v-model:image-url="uploadImageUrl" :drag="true">
						<template #empty>
...
watch(uploadImageUrl, async (newUrl: string) => {
	if (newUrl) {
		state.loading = true;
		await PicturesApi.add({ albumId: state.tableData.param.id, url: newUrl });
		await loadData();
		// 重置上传图片URL，以便下次上传
