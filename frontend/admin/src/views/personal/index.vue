				...
				<el-card shadow="hover" header="个人信息">
					<div class="personal-user">
						<div class="personal-user-left">
							...
							<!-- <UploadImg 
								v-model:image-url="model.info.avatar || ''"
								class="h100 personal-user-left-upload"
								@update:image-url="onAvatarChange"
							>
								<template #empty>
									<el-avatar :size="100">{{ model.info.name }}</el-avatar>
								</template>
							</UploadImg> -->
						</div>
...
const onAvatarChange = async (newUrl: string) => {
	if (newUrl) {
		const { succeeded } = await SysUserApi.setAvatar(newUrl);
		if (succeeded) {
